...
> **勘误（2026-08 架构审计）：** “26 个测试/451 断言”与仓库实际不符——当时仓库内仅
> ExampleTest×2、SecurityFixesTest、SingboxEchTest 四个测试文件，且 phpunit.xml 为 PHPUnit 9
> schema 与 composer 要求的 PHPUnit 11 不兼容，测试套件实际无法启动。测试脚手架已在
> 审计修复 FIX-09 中修复，核心链路测试在 FIX-01/02/04/05/06/12/13a 中补齐。
